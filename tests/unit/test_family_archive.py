...
def test_api_section_correspondents_search_and_sort(tmp_path):
    ...
    assert [r["address"] for r in unfiltered["rows"]] == ["bob@x.com", "dawn@yahoo.com", "dawn2@x.com"]
    by_name = case.api_section("correspondents", {"q": "DAWN"})
    assert by_name["total"] == 2
    assert {r["address"] for r in by_name["rows"]} == {"dawn@yahoo.com", "dawn2@x.com"}
    by_addr = case.api_section("correspondents", {"q": "yahoo"})
    assert by_addr["total"] == 1 and by_addr["rows"][0]["address"] == "dawn@yahoo.com"
    # ?sort=name / ?sort=recent re-order; total stays the count either way.
    by_name_sort = case.api_section("correspondents", {"sort": "name"})
    assert [r["name"] for r in by_name_sort["rows"]] == ["Bob Smith", "Dawn McNeill", "Dawn Someone"]
    by_recent = case.api_section("correspondents", {"sort": "recent"})
    assert [r["address"] for r in by_recent["rows"]] == ["bob@x.com", "dawn@yahoo.com", "dawn2@x.com"]
